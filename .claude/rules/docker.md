...
- **Default (no profile)** — infrastructure dependencies only (Postgres, Redis, RabbitMQ, etc.). `docker compose up` starts just the infra so devs can run apps locally with hot-reload.
- **`apps` profile** — application services (`web`, `api`, …). `docker compose --profile apps up` starts the full stack including built app containers.

Every app service must have `profiles: [apps]`. Infra services must **not** have a profile (they always start).
...
This is mandatory, not optional. In the **same change** where you:
...
The repo must always be launchable:

- `docker compose up` — infra only (default for local dev).
- `docker compose --profile apps up` — full stack including apps.
